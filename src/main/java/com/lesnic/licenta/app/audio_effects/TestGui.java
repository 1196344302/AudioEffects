...
public class TestGui extends Application {
    ...
    public void start(final Stage stage) throws Exception {
        ...
        // create some controls to manipulate the x and y pivot points of the
        // rotation.
        final Slider xPivotSlider = createSlider(50,
                "Best values are 0, 50 or 100");
        final Slider yPivotSlider = createSlider(50,
                "Best values are 0, 50 or 100");
        ...
        Node square;
        try { // grab a smurf from the path if it is there otherwise just use a
              // green square.
            square = new ImageView(
                    new Image(
                            "http://bluebuddies.com/gallery/title/jpg/Smurf_Fun_100x100.jpg"));
        } catch (Exception e) {
        ...
        // rotate a square using timeline attached to the rotation transform's
        // angle property.
        ...
        pivotMarker.translateXProperty().bind(
                xPivotSlider.valueProperty().subtract(50));
        pivotMarker.translateYProperty().bind(
                yPivotSlider.valueProperty().subtract(50));

        // display a dashed square border outline to mark the original location
        // of the square.
        ...
        xPivotControl.getChildren().addAll(new Label("X Pivot Point"),
                xPivotSlider);
        HBox.setHgrow(xPivotSlider, Priority.ALWAYS);

        final HBox yPivotControl = new HBox(5);
        yPivotControl.getChildren().addAll(new Label("Y Pivot Point"),
                yPivotSlider);
        HBox.setHgrow(yPivotSlider, Priority.ALWAYS);

        final HBox zPivotControl = new HBox(5);
        zPivotControl.getChildren().addAll(new Label("Z Pivot Point"),
                zPivotSlider);
        HBox.setHgrow(zPivotSlider, Priority.ALWAYS);

        final HBox axisControl = new HBox(20);
        axisControl.getChildren().addAll(new Label("Axis of Rotation"),
                xAxisToggleButton, yAxisToggleButton, zAxisToggleButton);
        ...
        layout.getChildren().addAll(
                VBoxBuilder
                        ...
                        .children(xPivotControl, yPivotControl, zPivotControl,
                                axisControl).build(), displayPane);
